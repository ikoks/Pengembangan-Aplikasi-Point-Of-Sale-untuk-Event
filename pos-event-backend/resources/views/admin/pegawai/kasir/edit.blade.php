...
<div class="bg-white brutal-border brutal-shadow p-6 max-w-xl">
    <form action="{{ route('admin.pegawai.kasir.update', $kasir->id_user) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="nama_user" value="{{ old('nama_user', $kasir->nama_user) }}" class="brutal-input" required>
        </div>
        
        <div class="mb-4">
            <label class="block font-extrabold mb-2 uppercase">Username</label>
            <input type="text" name="username" value="{{ old('username', $kasir->username) }}" class="brutal-input" required>
        </div>
            ...
            <div class="relative">
                <select name="id_cabang" class="brutal-input appearance-none bg-white">
                    ...
                    @foreach($cabangs as $cabang)
                        <option value="{{ $cabang->id_cabang }}" {{ old('id_cabang', $kasir->id_cabang) == $cabang->id_cabang ? 'selected' : '' }}>
                            {{ $cabang->nama_cabang }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="mb-6 flex items-center">
            <input type="checkbox" name="status_aktif" id="status_aktif" value="1" {{ old('status_aktif', $kasir->status_aktif) ? 'checked' : '' }} class="w-5 h-5 border-2 border-brutal-black rounded-none">
