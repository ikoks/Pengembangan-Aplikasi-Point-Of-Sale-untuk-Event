 ...
 <div x-data="{ open: {{ $errors->any() && !old('id_user') ? 'true' : 'false' }} }">
  ...
  <div x-show="open" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 text-left">
   <div @click.away="open = false" class="bg-white brutal-border brutal-shadow p-6 max-w-xl w-full">
    <h2 class="text-xl font-black mb-4 border-b-2 border-brutal-black pb-2">Tambah Admin</h2>
    <form action="{{ route('admin.management.store') }}" method="POST">
     @csrf
     <div class="mb-4">
      <label class="block font-extrabold mb-2 text-xs">Nama Lengkap</label>
     ...
     <div class="mb-4">
      <label class="block font-extrabold mb-2 text-xs">Username</label>
     ...
     <div class="mb-4">
      <label class="block font-extrabold mb-2 text-xs">Password</label>
     ...
     <div class="mb-6 flex items-center">
      ...
      <label for="status_aktif_create" class="ml-2 font-extrabold text-xs">Akun Aktif</label>
     </div>
    ...
    <td class="brutal-table-td text-center">
     <div class="flex justify-center items-center gap-2">
      ...
      <div x-data="{ open: {{ $errors->any() && old('id_user') == $admin->id_user ? 'true' : 'false' }} }" class="inline-block">
       ...
       <div x-show="open" style="display: none;" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 text-left">
        <div @click.away="open = false" class="bg-white brutal-border brutal-shadow p-6 max-w-xl w-full">
         <h2 class="text-xl font-black mb-4 border-b-2 border-brutal-black pb-2">Edit Admin</h2>
         <form action="{{ route('admin.management.update', $admin->id_user) }}" method="POST">
          ...
          <div class="mb-4">
           <label class="block font-extrabold mb-2 text-xs text-left">Password (opsional)</label>
          ...
          <div class="mb-6 flex items-center">
           ...
           <label for="status_aktif_edit_{{ $admin->id_user }}" class="ml-2 font-extrabold text-xs">Akun Aktif</label>
          </div>
          <div class="flex gap-4 mt-6">
           <button type="submit" class="brutal-btn brutal-btn-primary">Simpan</button>
           <button type="button" @click="open = false" class="brutal-btn brutal-btn-secondary">Batal</button>
