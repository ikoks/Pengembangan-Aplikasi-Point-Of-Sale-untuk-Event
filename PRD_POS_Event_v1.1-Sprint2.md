...
## Sistem POS Event — v2.2-Sprint7 (Fitur Menu Template Cabang, Refaktor Field Status & Penyempurnaan Log Shift)

**Status dokumen:** baseline aktif dan pembaruan hasil penyelesaian Fitur Menu Template Cabang, Refaktor Field Status (penghapusan dropdown di modal Edit), Penyempurnaan Shift Log, dan Bypass Pajak 0%  
**Tanggal audit:** 11 Agustus 2026  
**Platform:** Laravel 11 API + Web Admin; React Native 0.86 Android APK; MySQL; SQLite  
**Catatan versi:** `PRD_POS_Event_v1.0.md` dipertahankan sebagai arsip. Dokumen ini adalah baseline aktif yang diperbarui sesuai penambahan fitur Menu Template berbasis Mode Penjualan per Cabang, pembersihan UI (hapus form status dari modal Edit), dan penyempurnaan kalkulasi Shift Log.

## 1. Changelog

| Versi | Tanggal | Ringkasan |
|---|---:|---|
| v2.5-Sprint9 | 13 Agt 2026 | **Refaktor Manajemen Admin & Modal Cabang**: (1) Penyelesaian error `QueryException` terkait hilangnya kolom `deleted_at` pada tabel/model `SalesMode`. (2) Penghapusan fungsionalitas Reset Password manual pada antarmuka Manajemen Admin, mengandalkan fitur pemulihan via email. (3) Pembersihan struktur kode dengan konsolidasi file tampilan admin (`admin.blade.php` ke `admin/index.blade.php`). (4) Konversi dropdown native `<select>` menjadi komponen Pop-up Modal (Alpine.js) untuk input Mode Penjualan pada halaman Cabang (Create & Edit). |
| v2.4-Sprint8 | 12 Agt 2026 | **Fitur Input Manual Token QR Statis & API Resolusi Token**: (1) Penambahan kolom `qr_static_token` (VARCHAR 10) pada migrasi tabel `cabang` untuk menyediakan kode fallback 6 digit alfanumerik. (2) Pembaruan `CabangController` agar otomatis men-generate token dan menyematkannya ke dalam `qr_static_payload` berformat JSON. (3) Menambahkan tampilan visual token di `admin/cabang/index.blade.php` tepat di bawah tombol *Download*. (4) Pembuatan endpoint API publik `GET /api/v1/cabang/token/{token}` di `Api\V1\CabangController` agar aplikasi Kasir dapat memvalidasi token yang diinput manual ketika pemindaian QR gagal. |
| v2.3-Sprint8 | 12 Agt 2026 | **Integrasi Environment Kasir & Tunneling Ngrok Dinamis**: (1) Instalasi `react-native-dotenv` pada `PosEventKasir` untuk memisahkan konfigurasi URL API secara aman agar tidak masuk ke GitHub. (2) Pengaturan `apiClient.ts` untuk mengonsumsi `API_URL` dari file `.env` dengan *fallback* URL Ngrok statis. (3) Penambahan custom header `ngrok-skip-browser-warning` untuk bypass layar peringatan Ngrok. (4) Pembuatan file deklarasi TypeScript `env.d.ts` untuk `module:react-native-dotenv`. |
| v2.2-Sprint7 | 11 Agt 2026 | **Fitur Menu Template Cabang, Refaktor Field Status, & Penyempurnaan Log Shift**: (1) Penambahan kolom `id_sales` (FK ke `sales_mode`) pada tabel `cabang` melalui pembaruan file migrasi langsung, sehingga setiap cabang dapat memiliki mode penjualan *default* yang menentukan menu template-nya. (2) Implementasi fitur **Menu Template** pada halaman Cabang: modal global untuk melihat daftar menu sesuai mode penjualan yang dipilih cabang, disertai tombol **Edit Harga** yang mengarahkan ke halaman Harga Produk dengan filter mode penjualan aktif. (3) Refaktor Shift Log: menghapus kolom *Mode Penjualan* dari keterangan log dan mengganti label *Total Keuntungan* menjadi **Pendapatan**. (4) Penyempurnaan logika pajak: sistem otomatis melewati perhitungan pajak jika nilai pajak cabang di-set `0%` (bypass calculation), serta menambahkan validasi wajib isi jika field pajak dikosongkan. (5) **Hapus form dropdown Status** dari seluruh modal Edit di: Cabang, Kategori, Sub-Kategori, dan Kategori Metode Pembayaran — karena pengelolaan status telah sepenuhnya beralih ke mekanisme *toggle switch* langsung pada tabel. (6) Perbaikan tata letak (*layout*) modal Edit Cabang agar lebih rapi dengan susunan 2 kolom. |
| v2.1-Sprint6 | 8 Agt 2026 | **Modul Metode Pembayaran, Refinemen Shift Log & Pembersihan UI**: (1) Pembuatan halaman kelola "Metode Pembayaran" dan "Kategori Metode Pembayaran" dengan gaya desain Neo-Brutalist UI yang konsisten. (2) Perbaikan *route* halaman Riwayat Transaksi dan restrukturisasi *folder view*. (3) Refinemen perhitungan pada rincian "Shift Log" dengan pemisahan *Pendapatan Tunai* dan *Non-Tunai*, penambahan *Total Kas Sistem*, serta *Total Keuntungan* riil untuk mempermudah rekonsiliasi kas.(4) Penyesuaian visual warna *background* grafik Dasbor. |
