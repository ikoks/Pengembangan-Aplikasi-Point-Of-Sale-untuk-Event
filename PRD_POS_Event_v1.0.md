...
|---|---|---|---|
| `v1.4-Sprint2` | 22 Jul 2026 | TPM (AI) | Audit komprehensif backend: Penemuan implementasi CheckoutController@confirmTransaction (POS-5B) dan TransaksiController@index & show (POS-5D). Backend RESTful API mencapai 100% DONE (21/21 API), Total Proyek ~70%. |
| `v1.3-Sprint2` | 22 Jul 2026 | TPM (AI) | Audit codebase ShiftSessionController: Pembaruan status API Closing Shift (`POS-4B`), Jeda Shift Break/Resume (`POS-4C`), dan Switch Operator (`POS-4D`) dari BACKLOG ke DONE. Progress Backend API mencapai ~90.5%, Total Proyek ~65%. |
| `v1.2-Sprint2` | 22 Jul 2026 | TPM (AI) | Audit codebase komprehensif: Pembaruan status API Void & Cancel Transaksi/Item (`POS-5C` via `CancellationController`), verifikasi `CancelTransactionRequest` & `VoidItemRequest`, integrasi `AuditLogService`, pembaruan Live Feature Tracker & kalkulasi progress proyek (~59%). |
...
| Jeda shift (ON_BREAK) | `POST /api/v1/shift/break` & `/resume` | ✅ DONE |
| Menutup shift & rekonsiliasi | `POST /api/v1/shift/close` | ✅ DONE |
| Switch operator kasir | `POST /api/v1/shift/switch` | ✅ DONE |
...
| `POS-4B` | Closing Shift Kasir | API | ✅ **DONE** | `ShiftSessionController@close` — endpoint `POST /api/v1/shift/close` (hitung omzet tunai/non-tunai, ekspektasi & selisih laci, status CLOSED + audit log). |
| `POS-4C` | Jeda Shift (Break/Resume) | API | ✅ **DONE** | `ShiftSessionController@break` & `resume` — endpoint `POST /api/v1/shift/break` & `/resume` (transisi status OPEN ↔ ON_BREAK + audit log). |
| `POS-4D` | Switch Operator Kasir | API | ✅ **DONE** | `ShiftSessionController@switchOperator` — endpoint `POST /api/v1/shift/switch` (pengalihan kasir aktif terminal tanpa ubah pemilik shift + audit log). |
...
| `POS-5B` | Konfirmasi Transaksi → Success | API | ✅ **DONE** | `CheckoutController@confirmTransaction` — endpoint `POST /api/v1/checkout/{id}/confirm` (konfirmasi Draft/Pending ke Success + upsert detail_pembayaran_non_tunai). |
| `POS-5C` | Void/Cancel Transaksi | API | ✅ **DONE** | `CancellationController` — endpoint `POST /api/v1/checkout/{id}/cancel` (batal transaksi full + alasan_batal wajib + `AuditLogService`) & `POST /api/v1/checkout/{id}/void-item` (void item spesifik + re-kalkulasi PPN & total). |
| `POS-5D` | Riwayat & Detail Transaksi (API) | API | ✅ **DONE** | `TransaksiController@index` & `show` — endpoint `GET /api/v1/transaksi` & `/{id}` (pagination 15/page, filter shift/cabang/metode/status/tanggal, RBAC scoping Admin/Kasir). |
...
| **R-03** | **Kecurangan Void/Cancel** — Kasir melakukan void transaksi tanpa alasan valid untuk menutupi selisih kas. | 🟡 Sedang | 🔴 Kritis | **HIGH** | Kolom `alasan_batal` wajib diisi + `diperbarui_oleh` mencatat pelaku + audit log + supervisor approval flow. | ✅ **DONE** (`CancellationController` + `AuditLogService` + `CancelTransactionRequest`). |
...
| **R-05** | **Selisih Kas Saat Closing** — Uang fisik tidak cocok dengan total transaksi di sistem. | 🟡 Sedang | 🟡 Sedang | **MEDIUM** | Kolom `uang_fisik_akhir`, `selisih_uang` di `shift_session`. Alert jika selisih > threshold. Laporan per-shift. | ✅ **DONE** (`ShiftSessionController@close` kalkulasi otomatis omzet tunai vs non-tunai & `selisih_uang`). |
...
> **Terakhir diperbarui**: 22 Juli 2026, 11:15 WIB
> **Versi PRD**: v1.4-Sprint2
